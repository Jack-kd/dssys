...
final class ContinuationOutcomeReceiver<R, E extends Throwable> extends AtomicBoolean implements OutcomeReceiver {
    ...
    public ContinuationOutcomeReceiver(@NotNull kotlin.coroutines.e eVar) {
        super(false);
    ...
    public void onError(@NotNull E error) {
        if (compareAndSet(false, true)) {
            ...
            eVar.resumeWith(Result.m66constructorimpl(kotlin.e.a(error)));
        }
    }

    public void onResult(R result) {
        if (compareAndSet(false, true)) {
            this.continuation.resumeWith(Result.m66constructorimpl(result));
        }
    }
