...
public class ForceStopRunnable implements Runnable {
    ...
    public boolean isForceStopped() {
        try {
            ...
            PendingIntent pendingIntent = getPendingIntent(this.mContext, i2 >= 31 ? 570425344 : C.ENCODING_PCM_A_LAW);
            if (i2 >= 30) {
                ...
                if (historicalProcessExitReasons != null && !historicalProcessExitReasons.isEmpty()) {
                    ...
                    for (int i3 = 0; i3 < historicalProcessExitReasons.size(); i3++) {
                        ApplicationExitInfo applicationExitInfoA = b.a(historicalProcessExitReasons.get(i3));
                        if (applicationExitInfoA.getReason() == 10 && applicationExitInfoA.getTimestamp() >= lastForceStopEventMillis) {
                            return true;
                        }
                    }
                }
                    ...
                    try {
                        ...
                        try {
                            ...
                            if (i2 >= 3) {
                                ...
                                long j2 = i2 * BACKOFF_DURATION_MS;
                                Logger.get().debug(TAG, "Retrying after " + j2, e2);
                                sleep(((long) this.mRetryCount) * BACKOFF_DURATION_MS);
                            }
                        }
                        long j22 = i2 * BACKOFF_DURATION_MS;
                        Logger.get().debug(TAG, "Retrying after " + j22, e2);
                        sleep(((long) this.mRetryCount) * BACKOFF_DURATION_MS);
