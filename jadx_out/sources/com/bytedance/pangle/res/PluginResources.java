...
public class PluginResources extends Resources {
    ...
    public static AssetManager appendHostRes(Resources resources) {
        String strHp = eb.hp(Zeus.getAppApplication());
        String strL = eb.l(Zeus.getAppApplication());
        List<String> listL = gu.l();
        hp hpVar = new hp();
        AssetManager assets = resources.getAssets();
        AssetManager assets2 = Zeus.getAppApplication().getAssets();
        HashSet hashSet = new HashSet(gu.hp(assets));
        List<String> listHp = gu.hp(assets2);
        for (String str : listL) {
            if (!hashSet.contains(str)) {
                assets = hpVar.hp(assets, str, true);
            }
        }
        for (String str2 : listHp) {
            if (!isOtherPlugin(str2, strHp, strL) && !hashSet.contains(str2) && !listL.contains(str2)) {
                assets = hpVar.hp(assets, str2, false);
            }
        }
        ZeusLogger.i(ZeusLogger.TAG_RESOURCES, "pluginAssets = " + gu.l(assets));
        return assets;
    }

    private Resources.NotFoundException handleException(Resources.NotFoundException notFoundException) {
        return new Resources.NotFoundException(("Resources#Assets: " + gu.l(getAssets())) + "," + notFoundException.getMessage());
    }
