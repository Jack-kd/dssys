package com.kwad.sdk.core.b.a;

import com.anythink.core.common.f.g;
import com.anythink.core.common.f.s;
import com.kwad.components.ad.reward.monitor.RewardMonitorInfo;
import com.sigmob.sdk.base.mta.PointParamKey;
import com.smartdigimkt.sdk.api.format.SDMAdDeepRewardResult;
import org.json.JSONObject;

/* loaded from: classes4.dex */
public final class kx implements com.kwad.sdk.core.d<RewardMonitorInfo> {
    @Override // com.kwad.sdk.core.d
    public final /* bridge */ /* synthetic */ void a(com.kwad.sdk.core.b bVar, JSONObject jSONObject) {
        a((RewardMonitorInfo) bVar, jSONObject);
    }

    @Override // com.kwad.sdk.core.d
    public final /* bridge */ /* synthetic */ JSONObject b(com.kwad.sdk.core.b bVar, JSONObject jSONObject) {
        return b((RewardMonitorInfo) bVar, jSONObject);
    }

    private static void a(RewardMonitorInfo rewardMonitorInfo, JSONObject jSONObject) {
        if (jSONObject == null) {
            return;
        }
        rewardMonitorInfo.loadType = jSONObject.optInt("load_type");
        rewardMonitorInfo.loadStatus = jSONObject.optInt("load_status");
        rewardMonitorInfo.adCount = jSONObject.optInt(PointParamKey.AD_COUNT);
        rewardMonitorInfo.loadDataDuration = jSONObject.optLong("load_data_duration_ms");
        rewardMonitorInfo.downloadDuration = jSONObject.optLong("download_duration_ms");
        rewardMonitorInfo.totalDuration = jSONObject.optLong("total_duration_ms");
        rewardMonitorInfo.downloadType = jSONObject.optInt(g.a.f3275l);
        rewardMonitorInfo.downloadSize = jSONObject.optLong(s.a.f3403e);
        rewardMonitorInfo.creativeId = jSONObject.optLong("creative_id");
        rewardMonitorInfo.videoUrl = jSONObject.optString(s.a.f3400b);
        Object obj = JSONObject.NULL;
        if (obj.toString().equals(rewardMonitorInfo.videoUrl)) {
            rewardMonitorInfo.videoUrl = "";
        }
        rewardMonitorInfo.videoDuration = jSONObject.optLong("video_duration_ms");
        rewardMonitorInfo.dataLoadInterval = jSONObject.optLong("data_load_interval_duration_ms");
        rewardMonitorInfo.dataDownloadInterval = jSONObject.optLong("data_download_interval_duration_ms");
        rewardMonitorInfo.adSceneType = jSONObject.optInt("ad_scene_type");
        rewardMonitorInfo.renderDuration = jSONObject.optLong("render_duration_ms");
        rewardMonitorInfo.currentDuration = jSONObject.optLong("video_duration_ms");
        rewardMonitorInfo.pageStatus = jSONObject.optInt("page_status");
        rewardMonitorInfo.subStage = jSONObject.optString("sub_stage");
        if (obj.toString().equals(rewardMonitorInfo.subStage)) {
            rewardMonitorInfo.subStage = "";
        }
        rewardMonitorInfo.launchFrom = jSONObject.optString("launch_from");
        if (obj.toString().equals(rewardMonitorInfo.launchFrom)) {
            rewardMonitorInfo.launchFrom = "";
        }
        rewardMonitorInfo.rewardType = jSONObject.optInt("reward_type");
        rewardMonitorInfo.taskType = jSONObject.optInt(SDMAdDeepRewardResult.KEY_TASK_TYPE);
        rewardMonitorInfo.taskStep = jSONObject.optInt("task_step");
        rewardMonitorInfo.posId = jSONObject.optLong("pos_Id");
        rewardMonitorInfo.materialType = jSONObject.optInt("material_type");
        rewardMonitorInfo.materialRenderType = jSONObject.optString("material_render_type");
        if (obj.toString().equals(rewardMonitorInfo.materialRenderType)) {
            rewardMonitorInfo.materialRenderType = "";
        }
        rewardMonitorInfo.operationType = jSONObject.optInt("operation_type");
        rewardMonitorInfo.tkRenderType = jSONObject.optInt("tk_render_type");
        rewardMonitorInfo.callbackType = jSONObject.optString("callback_type");
        if (obj.toString().equals(rewardMonitorInfo.callbackType)) {
            rewardMonitorInfo.callbackType = "";
        }
        rewardMonitorInfo.enableRewardLayoutOptimise = jSONObject.optBoolean("enable_reward_layout_optimise");
        rewardMonitorInfo.enableFullscreenLayoutOptimise = jSONObject.optBoolean("enable_fullscreen_layout_optimise");
        rewardMonitorInfo.rewardTaskType = jSONObject.optInt("reward_task_type");
        rewardMonitorInfo.fraudVerifyType = jSONObject.optInt("fraud_verify_type");
        rewardMonitorInfo.fraudVerifyCode = jSONObject.optInt("fraud_verify_code");
        rewardMonitorInfo.interceptReason = jSONObject.optString("intercept_reason");
        if (obj.toString().equals(rewardMonitorInfo.interceptReason)) {
            rewardMonitorInfo.interceptReason = "";
        }
    }

    private static JSONObject b(RewardMonitorInfo rewardMonitorInfo, JSONObject jSONObject) {
        if (jSONObject == null) {
            jSONObject = new JSONObject();
        }
        int i2 = rewardMonitorInfo.loadType;
        if (i2 != 0) {
            com.kwad.sdk.utils.ac.putValue(jSONObject, "load_type", i2);
        }
        int i3 = rewardMonitorInfo.loadStatus;
        if (i3 != 0) {
            com.kwad.sdk.utils.ac.putValue(jSONObject, "load_status", i3);
        }
        int i4 = rewardMonitorInfo.adCount;
        if (i4 != 0) {
            com.kwad.sdk.utils.ac.putValue(jSONObject, PointParamKey.AD_COUNT, i4);
        }
        long j2 = rewardMonitorInfo.loadDataDuration;
        if (j2 != 0) {
            com.kwad.sdk.utils.ac.putValue(jSONObject, "load_data_duration_ms", j2);
        }
        long j3 = rewardMonitorInfo.downloadDuration;
        if (j3 != 0) {
            com.kwad.sdk.utils.ac.putValue(jSONObject, "download_duration_ms", j3);
        }
        long j4 = rewardMonitorInfo.totalDuration;
        if (j4 != 0) {
            com.kwad.sdk.utils.ac.putValue(jSONObject, "total_duration_ms", j4);
        }
        int i5 = rewardMonitorInfo.downloadType;
        if (i5 != 0) {
            com.kwad.sdk.utils.ac.putValue(jSONObject, g.a.f3275l, i5);
        }
        long j5 = rewardMonitorInfo.downloadSize;
        if (j5 != 0) {
            com.kwad.sdk.utils.ac.putValue(jSONObject, s.a.f3403e, j5);
        }
        long j6 = rewardMonitorInfo.creativeId;
        if (j6 != 0) {
            com.kwad.sdk.utils.ac.putValue(jSONObject, "creative_id", j6);
        }
        String str = rewardMonitorInfo.videoUrl;
        if (str != null && !str.equals("")) {
            com.kwad.sdk.utils.ac.putValue(jSONObject, s.a.f3400b, rewardMonitorInfo.videoUrl);
        }
        long j7 = rewardMonitorInfo.videoDuration;
        if (j7 != 0) {
            com.kwad.sdk.utils.ac.putValue(jSONObject, "video_duration_ms", j7);
        }
        long j8 = rewardMonitorInfo.dataLoadInterval;
        if (j8 != 0) {
            com.kwad.sdk.utils.ac.putValue(jSONObject, "data_load_interval_duration_ms", j8);
        }
        long j9 = rewardMonitorInfo.dataDownloadInterval;
        if (j9 != 0) {
            com.kwad.sdk.utils.ac.putValue(jSONObject, "data_download_interval_duration_ms", j9);
        }
        int i6 = rewardMonitorInfo.adSceneType;
        if (i6 != 0) {
            com.kwad.sdk.utils.ac.putValue(jSONObject, "ad_scene_type", i6);
        }
        long j10 = rewardMonitorInfo.renderDuration;
        if (j10 != 0) {
            com.kwad.sdk.utils.ac.putValue(jSONObject, "render_duration_ms", j10);
        }
        long j11 = rewardMonitorInfo.currentDuration;
        if (j11 != 0) {
            com.kwad.sdk.utils.ac.putValue(jSONObject, "video_duration_ms", j11);
        }
        int i7 = rewardMonitorInfo.pageStatus;
        if (i7 != 0) {
            com.kwad.sdk.utils.ac.putValue(jSONObject, "page_status", i7);
        }
        String str2 = rewardMonitorInfo.subStage;
        if (str2 != null && !str2.equals("")) {
            com.kwad.sdk.utils.ac.putValue(jSONObject, "sub_stage", rewardMonitorInfo.subStage);
        }
        String str3 = rewardMonitorInfo.launchFrom;
        if (str3 != null && !str3.equals("")) {
            com.kwad.sdk.utils.ac.putValue(jSONObject, "launch_from", rewardMonitorInfo.launchFrom);
        }
        int i8 = rewardMonitorInfo.rewardType;
        if (i8 != 0) {
            com.kwad.sdk.utils.ac.putValue(jSONObject, "reward_type", i8);
        }
        int i9 = rewardMonitorInfo.taskType;
        if (i9 != 0) {
            com.kwad.sdk.utils.ac.putValue(jSONObject, SDMAdDeepRewardResult.KEY_TASK_TYPE, i9);
        }
        int i10 = rewardMonitorInfo.taskStep;
        if (i10 != 0) {
            com.kwad.sdk.utils.ac.putValue(jSONObject, "task_step", i10);
        }
        long j12 = rewardMonitorInfo.posId;
        if (j12 != 0) {
            com.kwad.sdk.utils.ac.putValue(jSONObject, "pos_Id", j12);
        }
        int i11 = rewardMonitorInfo.materialType;
        if (i11 != 0) {
            com.kwad.sdk.utils.ac.putValue(jSONObject, "material_type", i11);
        }
        String str4 = rewardMonitorInfo.materialRenderType;
        if (str4 != null && !str4.equals("")) {
            com.kwad.sdk.utils.ac.putValue(jSONObject, "material_render_type", rewardMonitorInfo.materialRenderType);
        }
        int i12 = rewardMonitorInfo.operationType;
        if (i12 != 0) {
            com.kwad.sdk.utils.ac.putValue(jSONObject, "operation_type", i12);
        }
        int i13 = rewardMonitorInfo.tkRenderType;
        if (i13 != 0) {
            com.kwad.sdk.utils.ac.putValue(jSONObject, "tk_render_type", i13);
        }
        String str5 = rewardMonitorInfo.callbackType;
        if (str5 != null && !str5.equals("")) {
            com.kwad.sdk.utils.ac.putValue(jSONObject, "callback_type", rewardMonitorInfo.callbackType);
        }
        boolean z2 = rewardMonitorInfo.enableRewardLayoutOptimise;
        if (z2) {
            com.kwad.sdk.utils.ac.putValue(jSONObject, "enable_reward_layout_optimise", z2);
        }
        boolean z3 = rewardMonitorInfo.enableFullscreenLayoutOptimise;
        if (z3) {
            com.kwad.sdk.utils.ac.putValue(jSONObject, "enable_fullscreen_layout_optimise", z3);
        }
        int i14 = rewardMonitorInfo.rewardTaskType;
        if (i14 != 0) {
            com.kwad.sdk.utils.ac.putValue(jSONObject, "reward_task_type", i14);
        }
        int i15 = rewardMonitorInfo.fraudVerifyType;
        if (i15 != 0) {
            com.kwad.sdk.utils.ac.putValue(jSONObject, "fraud_verify_type", i15);
        }
        int i16 = rewardMonitorInfo.fraudVerifyCode;
        if (i16 != 0) {
            com.kwad.sdk.utils.ac.putValue(jSONObject, "fraud_verify_code", i16);
        }
        String str6 = rewardMonitorInfo.interceptReason;
        if (str6 != null && !str6.equals("")) {
            com.kwad.sdk.utils.ac.putValue(jSONObject, "intercept_reason", rewardMonitorInfo.interceptReason);
        }
        return jSONObject;
    }
}
