...
public class g extends b<n, f> {
    ...
    public f G(List<n> list) {
        ...
        String strJ = J(list);
        return !TextUtils.isEmpty(strJ) ? new f(strJ) : new f(list);
    }
    ...
    private static boolean an(long j2) {
        s sVar = (s) ServiceProvider.get(s.class);
        return sVar != null && sVar.an(j2);
    }

    private void b(final n nVar, boolean z2) {
        if (nVar == null || !aVk) {
            return;
        }
        if (z2 || an(nVar.actionType)) {
    ...
    public static void a(@NonNull n nVar, boolean z2) {
        MZ().b(nVar, z2);
    }
