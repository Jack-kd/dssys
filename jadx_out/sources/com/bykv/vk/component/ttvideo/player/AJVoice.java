...
public class AJVoice implements AudioTrack.OnPlaybackPositionUpdateListener {
    ...
    private long getPlaybackHeadPosition() {
        ...
        long playbackHeadPosition = this.mAudioTrack.getPlaybackHeadPosition() & KeyboardMap.kValueMask;
    ...
    private int reconfigure() {
        ...
        if (minBufferSize <= 0) {
            u.l(TAG, "getMinBufferSize failed, trace: sampleRate = " + this.mSampleRate + ", mChannelsLayout = " + this.mChannelsLayout + ", mAudioFormat = " + this.mAudioFormat);
        ...
        try {
            ...
            try {
                ...
                u.l(TAG, "create audiotrack but failed to initialize");
                return -3;
            } catch (Throwable th) {
                u.l(TAG, "create audio track failed ,detail = ".concat(String.valueOf(th)));
                return -1;
            }
        } catch (OutOfMemoryError unused2) {
            u.l(TAG, "out of memory error when new audio buffer for audiotrack");
            return -10;
        }
    }
    ...
    public void resume() {
        try {
            ...
            if (audioTrackPositionTracker != null) {
                ...
                audioTrackPositionTracker.setAudioTrack(audioTrack, i2, getPcmFrameSize(i2, this.mChannels), this.mTrackBufferSize);
    ...
    public void setVolume(float f2, float f3) {
        ...
        try {
            ...
            if (f3 != -1048575.0f || (audioTrack = this.mAudioTrack) == null) {
                AudioManager audioManager = this.mAudioManager;
                if (audioManager != null) {
                    audioManager.setStreamVolume(this.mStreamType, (int) f2, 0);
                }
            } else {
                audioTrack.setStereoVolume(f2, f3);
            }
    ...
    public int start() {
        if (this.mAudioTrack == null) {
            u.l(TAG, "audiotrack start before created");
            return -2;
        }
        this.mStoped = false;
        try {
            ...
            u.l(TAG, "audioTrack start failed = ".concat(String.valueOf(th)));
            return -3;
        }
    }
    ...
    public int write(byte[] bArr, int i2, int i3, int i4) {
        ...
        try {
            int iWrite = this.mAudioTrack.write(bArr, i2, i3);
            if (iWrite >= 0) {
            ...
            u.l(TAG, "write failed : ret: " + iWrite + ", size = " + i3);
            return iWrite;
        } catch (Exception unused) {
            return -1;
        }
    ...
    @CalledByNative
    public int write(byte[] bArr, int i2, int i3, int i4, long j2) throws IllegalStateException {
        if (bArr != null && bArr.length != 0) {
            int iStart = 0;
            if (this.mStoped) {
                return 0;
            ...
            if (i5 == -1) {
                this.mStartMediaTimeMs = (int) Math.max(0L, j2);
            } else {
                long writtenDurationMs = i5 + getWrittenDurationMs();
                if (Math.abs(writtenDurationMs - j2) > FORCE_RESET_WORKAROUND_TIMEOUT_MS) {
                    u.l(TAG, "Discontinuity detected [expected " + writtenDurationMs + ", got " + j2 + "]");
                    this.mStartMediaTimeMs = (int) (((long) this.mStartMediaTimeMs) + (j2 - writtenDurationMs));
                }
            }
            try {
                int iWrite = this.mAudioTrack.write(bArr, i2, i3);
