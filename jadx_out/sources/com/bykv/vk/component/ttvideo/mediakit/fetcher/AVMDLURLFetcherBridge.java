...
public class AVMDLURLFetcherBridge implements AVMDLURLFetcherListener {
    ...
    public void onCompletion(int i2, String str, String str2, String[] strArr) {
        ...
        try {
            if (this.isFinish) {
                String.format(Locale.ENGLISH, "has fetch finished not need cur completion code:%d result:%s", Integer.valueOf(i2), this.newURL);
            } else {
                if (strArr != null && strArr.length > 0) {
                    this.newURL = strArr[0];
                }
                String.format(Locale.ENGLISH, "receive completion code:%d result:%s", Integer.valueOf(i2), this.newURL);
                this.isFinish = true;
                long j2 = this.handle;
                if (j2 != 0) {
                    _notifyFetcherResult(j2, str, str2, this.newURL);
                }
            }
            this.lock.unlock();
        } catch (Throwable th) {
            ...
            if (iStart > 0) {
                ...
                if (uRLs != null && uRLs.length != 0) {
                    ...
                    if (j3 != 0) {
                        String str4 = uRLs[0];
                        this.newURL = str4;
                        _notifyFetcherResult(j3, str, str2, str4);
                        i2 = 1;
                    }
                }
