...
import com.byazt.nw.j;
import java.util.Iterator;
import java.util.List;
import org.json.JSONException;
import org.json.JSONObject;

@com.byazt.kj.hp(hp = {0, 1, 1283, 30})
/* loaded from: classes3.dex */
public class jx {
    public JSONObject hp(List<j.hp> list, int i2, JSONObject jSONObject) {
        j.hp next;
        JSONObject jSONObject2;
        if (list == null || list.size() <= 0) {
            return null;
        }
        Iterator<j.hp> it = list.iterator();
        while (true) {
            if (!it.hasNext()) {
                next = null;
                break;
            ...
            next = it.next();
            if (next != null && next.hp == i2) {
                break;
            }
        }
        if (next == null || (jSONObject2 = next.f23840l) == null) {
            return null;
        }
        return hp(jSONObject2, jSONObject);
    }

    private static JSONObject hp(JSONObject jSONObject, JSONObject jSONObject2) throws JSONException {
