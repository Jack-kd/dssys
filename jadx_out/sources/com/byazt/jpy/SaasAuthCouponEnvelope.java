package com.byazt.jpy;

import android.content.Context;
import android.graphics.Color;
import android.widget.ImageView;
import android.widget.LinearLayout;
import android.widget.RelativeLayout;
import android.widget.TextView;
import androidx.annotation.NonNull;
import com.byazt.ij.jx;
import com.byazt.kj.hp;
import com.byazt.ws.l;
import com.byazt.xci.as;
import com.byazt.zn.xh;
import com.bykv.vk.component.ttvideo.player.MediaPlayer;

@hp(hp = {0, 1, MediaPlayer.MEDIA_PLAYER_OPTION_CMAF_CONNECT_TIME, 2287})
/* loaded from: classes.dex */
public class SaasAuthCouponEnvelope extends BaseSaasEnvelope {
    public TextView hp;

    public SaasAuthCouponEnvelope(@NonNull Context context, as.l lVar) {
        super(context);
        hp(context, lVar);
    }

    private void hp(Context context, as.l lVar) {
        if (lVar == null) {
            setVisibility(8);
            return;
        }
        RelativeLayout relativeLayoutHp = hp(context);
        ImageView imageView = new ImageView(context);
        try {
            l.hp(jx.hp("saas_reward_coupon_bg.webp")).to(imageView);
        } catch (Error unused) {
        }
        imageView.setScaleType(ImageView.ScaleType.FIT_XY);
        relativeLayoutHp.addView(imageView, new RelativeLayout.LayoutParams(-1, -1));
        TextView textView = new TextView(context);
        textView.setId(2114387802);
        RelativeLayout.LayoutParams layoutParams = new RelativeLayout.LayoutParams(-2, -2);
        layoutParams.addRule(14);
        textView.setLayoutParams(layoutParams);
        layoutParams.topMargin = xh.jx(context, 20.0f);
        textView.setText("优惠券");
        textView.setTextColor(Color.parseColor("#A9512C"));
        textView.setTextSize(2, 16.0f);
        relativeLayoutHp.addView(textView);
        LinearLayout linearLayout = new LinearLayout(context);
        linearLayout.setId(2114387461);
        linearLayout.setOrientation(0);
        RelativeLayout.LayoutParams layoutParams2 = new RelativeLayout.LayoutParams(-2, -2);
        layoutParams2.addRule(14);
        layoutParams2.addRule(3, textView.getId());
        linearLayout.setLayoutParams(layoutParams2);
        TextView textView2 = new TextView(context);
        this.hp = textView2;
        textView2.setId(2114387460);
        this.hp.setText(String.valueOf(lVar.hp()));
        this.hp.setTextColor(Color.parseColor("#F93F3F"));
        this.hp.setTextSize(2, 60.0f);
        linearLayout.addView(this.hp, new RelativeLayout.LayoutParams(-2, -2));
        TextView textView3 = new TextView(context);
        textView3.setText("元");
        textView3.setTextColor(Color.parseColor("#F93F3F"));
        textView3.setTextSize(2, 20.0f);
        linearLayout.addView(textView3, new RelativeLayout.LayoutParams(-2, -2));
        relativeLayoutHp.addView(linearLayout);
        TextView textView4 = new TextView(context);
        textView4.setId(2114387459);
        textView4.setText("满" + lVar.l() + "元可用");
        textView4.setTextColor(Color.parseColor("#F93F3F"));
        textView4.setTextSize(2, 16.0f);
        RelativeLayout.LayoutParams layoutParams3 = new RelativeLayout.LayoutParams(-2, -2);
        layoutParams3.addRule(14);
        layoutParams3.addRule(3, linearLayout.getId());
        layoutParams3.topMargin = xh.jx(context, -4.0f);
        relativeLayoutHp.addView(textView4, layoutParams3);
        TextView textView5 = new TextView(context);
        textView5.setText("领取后30分钟内有效");
        textView5.setTextColor(Color.parseColor("#894200"));
        textView5.setTextSize(2, 14.0f);
        RelativeLayout.LayoutParams layoutParams4 = new RelativeLayout.LayoutParams(-2, -2);
        layoutParams4.addRule(14);
        layoutParams4.addRule(3, textView4.getId());
        layoutParams4.topMargin = xh.jx(context, 8.0f);
        relativeLayoutHp.addView(textView5, layoutParams4);
        hp(context, "一键关联抖音领取", relativeLayoutHp);
        addView(relativeLayoutHp);
    }

    @Override // com.byazt.jpy.BaseSaasEnvelope
    public void l() {
        setVisibility(8);
        super.l();
    }

    @Override // com.byazt.jpy.BaseSaasEnvelope
    public void hp() {
        setVisibility(0);
    }
}
