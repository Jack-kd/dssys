...
public class aw {
    ...
    private JSONObject f30827a;

    public aw(Context context) {
        if (Engine.loadSuccess) {
            try {
                String strPqr = Engine.getInstance(context).pqr(Integer.valueOf(ci.f30964h).intValue(), 1, 0, "");
                if (TextUtils.isEmpty(strPqr)) {
                    return;
                }
                this.f30827a = new JSONObject(strPqr);
    ...
    public String a(String str) {
        JSONObject jSONObject = this.f30827a;
        if (jSONObject != null) {
            try {
        ...
        try {
            ...
            for (int i3 = 0; i3 < jSONArray.length(); i3++) {
                ...
                if (!string2.contains("azeroth") && !string2.contains("OkHttp") && !string2.contains("retrofit")) {
                    ...
                    if (!strSubstring.contains(packageName) && !packageName.contains(strSubstring) && !setA.contains(strSubstring2) && !b(i2).contains(strSubstring2)) {
                        hashSet.add(strSubstring);
                    }
                }
            }
