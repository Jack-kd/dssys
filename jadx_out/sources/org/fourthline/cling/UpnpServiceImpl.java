...
public class UpnpServiceImpl implements UpnpService {
    ...
    public void shutdownRouter() throws IllegalArgumentException {
        try {
            getRouter().shutdown();
        } catch (RouterException e2) {
            Throwable thUnwrap = Exceptions.unwrap(e2);
            if (thUnwrap instanceof InterruptedException) {
                log.log(Level.INFO, "Router shutdown was interrupted: " + e2, thUnwrap);
                return;
            }
            log.log(Level.SEVERE, "Router error on shutdown: " + e2, thUnwrap);
        }
    }
