...
public abstract class AbstractPeeringConnectionManagerService extends ConnectionManagerService {
    ...
    public synchronized void closeConnectionWithPeer(ControlPoint controlPoint, Service service, int i2) throws ActionException {
        closeConnectionWithPeer(controlPoint, service, getCurrentConnectionInfo(i2));
    }

    @UpnpAction
    public synchronized void connectionComplete(@UpnpInputArgument(name = "ConnectionID", stateVariable = "A_ARG_TYPE_ConnectionID") int i2) throws ActionException {
        ConnectionInfo currentConnectionInfo = getCurrentConnectionInfo(i2);
        log.fine("Closing connection ID " + i2);
        closeConnection(currentConnectionInfo);
        removeConnection(i2);
    }

    public abstract ConnectionInfo createConnection(int i2, int i3, ServiceReference serviceReference, ConnectionInfo.Direction direction, ProtocolInfo protocolInfo) throws ActionException;
    ...
    public synchronized int createConnectionWithPeer(ServiceReference serviceReference, ControlPoint controlPoint, final Service service, final ProtocolInfo protocolInfo, final ConnectionInfo.Direction direction) {
        ...
        new PrepareForConnection(service, controlPoint, protocolInfo, serviceReference, newConnectionId, direction) { // from class: org.fourthline.cling.support.connectionmanager.AbstractPeeringConnectionManagerService.1
            ...
            public void received(ActionInvocation actionInvocation, int i2, int i3, int i4) {
                AbstractPeeringConnectionManagerService.this.storeConnection(new ConnectionInfo(newConnectionId, i3, i4, protocolInfo, service.getReference(), i2, direction.getOpposite(), ConnectionInfo.Status.OK));
            }
        }.run();
        if (zArr[0]) {
            newConnectionId = -1;
    ...
    public synchronized ConnectionInfo prepareForConnection(@UpnpInputArgument(name = "RemoteProtocolInfo", stateVariable = "A_ARG_TYPE_ProtocolInfo") ProtocolInfo protocolInfo, @UpnpInputArgument(name = "PeerConnectionManager", stateVariable = "A_ARG_TYPE_ConnectionManager") ServiceReference serviceReference, @UpnpInputArgument(name = "PeerConnectionID", stateVariable = "A_ARG_TYPE_ConnectionID") int i2, @UpnpInputArgument(name = "Direction", stateVariable = "A_ARG_TYPE_Direction") String str) throws Throwable {
        try {
            try {
                int newConnectionId = getNewConnectionId();
                try {
                    ConnectionInfo.Direction directionValueOf = ConnectionInfo.Direction.valueOf(str);
                    log.fine("Preparing for connection with local new ID " + newConnectionId + " and peer connection ID: " + i2);
                    ConnectionInfo connectionInfoCreateConnection = createConnection(newConnectionId, i2, serviceReference, directionValueOf, protocolInfo);
    ...
    public synchronized void removeConnection(int i2) {
        ...
        this.activeConnections.remove(Integer.valueOf(i2));
        log.fine("Connection removed, firing event: " + i2);
