...
public class CoreLogCategories extends ArrayList<LogCategory> {
    public CoreLogCategories() {
        ...
        LogCategory.Group group = new LogCategory.Group("UDP communication", new LogCategory.LoggerLevel[]{new LogCategory.LoggerLevel(name, level), new LogCategory.LoggerLevel(MulticastReceiver.class.getName(), level)});
        String name2 = DatagramProcessor.class.getName();
        Level level2 = Level.FINER;
        add(new LogCategory("Network", new LogCategory.Group[]{group, new LogCategory.Group("UDP datagram processing and content", new LogCategory.LoggerLevel[]{new LogCategory.LoggerLevel(name2, level2)}), new LogCategory.Group("TCP communication", new LogCategory.LoggerLevel[]{new LogCategory.LoggerLevel(UpnpStream.class.getName(), level2), new LogCategory.LoggerLevel(StreamServer.class.getName(), level), new LogCategory.LoggerLevel(StreamClient.class.getName(), level)}), new LogCategory.Group("SOAP action message processing and content", new LogCategory.LoggerLevel[]{new LogCategory.LoggerLevel(SOAPActionProcessor.class.getName(), level2)}), new LogCategory.Group("GENA event message processing and content", new LogCategory.LoggerLevel[]{new LogCategory.LoggerLevel(GENAEventProcessor.class.getName(), level2)}), new LogCategory.Group("HTTP header processing", new LogCategory.LoggerLevel[]{new LogCategory.LoggerLevel(UpnpHeaders.class.getName(), level2)})}));
