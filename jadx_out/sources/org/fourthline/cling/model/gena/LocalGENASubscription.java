...
public abstract class LocalGENASubscription extends GENASubscription<LocalService> implements PropertyChangeListener {
    ...
    public synchronized void end(CancelReason cancelReason) {
        try {
            ...
            log.warning("Removal of local service property change listener failed: " + Exceptions.unwrap(e2));
        }
        ended(cancelReason);
    }
    ...
    public synchronized Set<String> moderateStateVariables(long j2, Collection<StateVariableValue> collection) {
        HashSet hashSet;
        try {
            hashSet = new HashSet();
            for (StateVariableValue stateVariableValue : collection) {
                ...
                } else if (stateVariable.getEventDetails().getEventMaximumRateMilliseconds() > 0 && j2 <= this.lastSentTimestamp.get(name).longValue() + stateVariable.getEventDetails().getEventMaximumRateMilliseconds()) {
