package org.seamless.util;

import androidx.core.text.util.LocalePreferences;
import com.byazt.t.hp;
import com.sigmob.sdk.base.mta.PointCategory;

/* loaded from: classes5.dex */
public class OS {
    public static boolean checkForHp() {
        return checkForPresence("os.name", hp.hp);
    }

    public static boolean checkForLinux() {
        return checkForPresence("os.name", "linux");
    }

    public static boolean checkForMac() {
        return checkForPresence("os.name", "mac");
    }

    private static boolean checkForPresence(String str, String str2) {
        try {
            String property = System.getProperty(str);
            if (property != null) {
                if (property.trim().toLowerCase().startsWith(str2)) {
                    return true;
                }
            }
        } catch (Throwable unused) {
        }
        return false;
    }

    public static boolean checkForSolaris() {
        return checkForPresence("os.name", LocalePreferences.FirstDayOfWeek.SUNDAY);
    }

    public static boolean checkForWindows() {
        return checkForPresence("os.name", PointCategory.WIN);
    }
}
