package XI.XI.K0;

import XI.K0.XI.XI;
import XI.xo.XI.XI.CA;
import android.content.Context;
import android.content.pm.PackageInfo;
import android.content.pm.PackageManager;
import android.os.Build;
import java.lang.reflect.Method;

/* loaded from: classes.dex */
public class XI {

    /* renamed from: K0, reason: collision with root package name */
    public static boolean f704K0 = false;

    /* renamed from: XI, reason: collision with root package name */
    public static boolean f705XI = false;

    public static Context CA(Context context) {
        return (context == null || context.getApplicationContext() == null) ? context : context.getApplicationContext();
    }

    public static String K0(Context context) {
        XI.CA.XI.K0 k0XI = XI.CA.XI.K0.XI(context);
        if (k0XI == null) {
            return null;
        }
        String str = XI.CA.XI.K0.WI;
        if (str != null) {
            return str;
        }
        k0XI.XI(0, null);
        if (XI.CA.XI.K0.kM == null) {
            XI.CA.XI.K0.XI(XI.CA.XI.K0.f691XI, 0, null);
        }
        return XI.CA.XI.K0.WI;
    }

    public static final boolean XI() throws NoSuchMethodException, SecurityException {
        Context context = null;
        try {
            Method method = Class.forName("android.app.ActivityThread").getMethod("currentApplication", null);
            method.setAccessible(true);
            context = (Context) method.invoke(null, null);
        } catch (Exception e2) {
            e2.toString();
        }
        if (context == null) {
            return false;
        }
        return CA.XI().XI(context, false);
    }

    public static String kM(Context context) {
        XI.CA.XI.K0 k0XI = XI.CA.XI.K0.XI(context);
        if (k0XI == null || Build.VERSION.SDK_INT == 28) {
            return null;
        }
        String str = XI.CA.XI.K0.bs;
        if (str != null) {
            return str;
        }
        String packageName = XI.CA.XI.K0.f691XI.getPackageName();
        k0XI.XI(1, packageName);
        if (XI.CA.XI.K0.xo == null && XI.CA.XI.K0.bs != null) {
            XI.CA.XI.K0.XI(XI.CA.XI.K0.f691XI, 1, packageName);
        }
        return XI.CA.XI.K0.bs;
    }

    public static void xo(Context context) throws PackageManager.NameNotFoundException {
        if (f705XI) {
            return;
        }
        XI.K0 k02 = XI.K0.C0013K0.f697XI;
        Context contextCA = CA(context);
        k02.getClass();
        boolean z2 = false;
        try {
            PackageInfo packageInfo = contextCA.getPackageManager().getPackageInfo("com.heytap.openid", 0);
            if (packageInfo != null) {
                if (packageInfo.versionCode >= 1) {
                    z2 = true;
                }
            }
        } catch (PackageManager.NameNotFoundException unused) {
        }
        f704K0 = z2;
        f705XI = true;
    }

    public static String K0(Context context, String str) {
        XI.CA.XI.K0 k0XI = XI.CA.XI.K0.XI(context);
        if (k0XI == null) {
            return null;
        }
        String str2 = XI.CA.XI.K0.bs;
        if (str2 != null) {
            return str2;
        }
        k0XI.XI(1, str);
        if (XI.CA.XI.K0.xo == null && XI.CA.XI.K0.bs != null) {
            XI.CA.XI.K0.XI(XI.CA.XI.K0.f691XI, 1, str);
        }
        return XI.CA.XI.K0.bs;
    }

    public static String XI(Context context, String str) {
        XI.CA.XI.K0 k0XI = XI.CA.XI.K0.XI(context);
        if (k0XI == null) {
            return null;
        }
        String str2 = XI.CA.XI.K0.cs;
        if (str2 != null) {
            return str2;
        }
        k0XI.XI(2, str);
        if (XI.CA.XI.K0.CA == null && XI.CA.XI.K0.cs != null) {
            XI.CA.XI.K0.XI(XI.CA.XI.K0.f691XI, 2, str);
        }
        return XI.CA.XI.K0.cs;
    }

    public static String XI(Context context) {
        XI.CA.XI.K0 k0XI = XI.CA.XI.K0.XI(context);
        if (k0XI == null || Build.VERSION.SDK_INT == 28) {
            return null;
        }
        String str = XI.CA.XI.K0.cs;
        if (str != null) {
            return str;
        }
        String packageName = XI.CA.XI.K0.f691XI.getPackageName();
        k0XI.XI(2, packageName);
        if (XI.CA.XI.K0.CA == null && XI.CA.XI.K0.cs != null) {
            XI.CA.XI.K0.XI(XI.CA.XI.K0.f691XI, 2, packageName);
        }
        return XI.CA.XI.K0.cs;
    }
}
